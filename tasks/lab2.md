...
### Ведение журнала
 - При загрузке плагина: INFO "Plugin %s loaded", ${имя_плагина}
...
### Обработка ошибок
 ...
 - Отсутсвие наддиректории для директории журнала: сообщение "The path to the log directory is missing: %s", ${ошибочный_путь_до_родительской_директории_над_директорией_логов} в лог в stderr; Все последующие логинаправляются в stderr;
 - Ошибка открытия файла конфигурации: "File %s not found", ${ошибочный_путь_до_файла_конфигурации}, должны быть проинициализированы стандартные значения или значения из других ресурсов, если такие существуют
 - Ошибка прав доступа: "Have no permissions for file %s", ${путь_до_файла}
