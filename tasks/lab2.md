...
1. Утилиты `mount` и `umount`. Что они делают и зачем нужны?
1. Что такое переменные окружения? зачем они нужны? где они находятся?
1. В чём различие между `interrupt`, `syscall`, `sysenter`?


## Список литературы:
1. "Understanding Linux Kernel"
1. [Лекции А.Л. Пережогина в записи А.А. Багаева и Е.В. Казаковой](https://sites.google.com/view/nsumedia/main_potok/second_course/third_semester/discrete_math?authuser=0) см. главы про конечные автоматы-распознаватели


## Задания (порядок выполнения не важен)

1. Логгер:
    1. Реализуйте динамическую библиотеку логгера с описанным ниже [интерфейсом](#файл-loggerh).
...
1. Конфиг:
    1. Реализуйте статическую библиотеку системы конфигурации с указанным ниже [интерфейсом](#файл-configh).
...
1. Время:
    1. Реализуйте динамическую библиотеку для времени с указанным ниже [интерфейсом](#файл-my_timeh). Для этого используйте функцию `time(2)`.
...
1. При реализации учтите возможные способы передачи пути к файлу с логами (см. [раздел](#команды-для-запуска-и-окружение) "Команды для запуска
...
то запускается режим отладки (смотрите [пункт](#задания-порядок-выполнения-не-важен) "Режим отладки")

### Ведение журнала

1. При успешном открытии файла конфигурации:
    ```C
    LOG_INFO "Start read config file %s", ${путь_до_файла_конфигурации}
    ```
1. При успешном завершении чтения файла конфигурации:
    ```C
    LOG_INFO "Finish read config file %s", ${путь_до_файла_конфигурации}
    ```
1. При загрузке плагина:
    ```C
    LOG_INFO "Plugin %s loaded", ${имя_плагина}
    ```
...
1. Отсутствие прав доступа к файлу(например файлу журналов):
    ```C
    "Have no permissions for file %s", ${путь_до_файла}
    ```
1. Файл конфигурации не найден по заданному пути: сообщение
    ```C
    "File %s not found", ${ошибочный_путь_до_файла_конфигурации}
    ```
    должны быть проинициализированы стандартные значения или значения из других источников (например флага командной строки), если такие существуют
1. В файле конфигурации синтаксическая ошибка: сообщение
    ```C
    "Configuration file %s has syntax error at %d: %s", ${путь_до_файла, номер_строки_ошибки, строка_с_ошибкой}
    ```
    после этого переход к следующей строке файла конфигурации.
...
1. Через переменную окружения `PROXY_CONFIG_PATH`
...
1. Через переменную файла конфигурирования `logs`

Передача списка плагинов для загрузки возможна двумя способами:
1. Через переменную `plugins` в файле конфигурации
1. Через переменную окружения `PROXY_MASTER_PLUGINS` формат: ```plugin1, plugin2, plugin3```, где pluginN - название плагина, которое возвращает name().
...
В случае возниконовения каких-либо ошибок в функции `write_log` вывод сообщения должен производиться в
стандартный поток (STDERR).

> [!TIP]
> При этом обрабатывать возвращаемое функцией `write_log` значение (как и в случае функции `printf`)
>есть необхоимость только в процессе отладки (то есть если при разработке возникли проблемы). В остальных
>случаях **возвращаемое значение можно игнорировать**

> [!TIP]
> Вместо прямого вызова `write_log` используйте макросы чтобы постоянно не указывать все аргументы.
>Это можно сделать например так:
>```
>#define LOG1(stream, level, format, ...) \
>    do { write_log(stream, level, __FILE__, __LINE__, format, ##__VA_ARGS__); } while (0)
>```
...
Ключ должен состоять из следующих символов: A-Z, a-z, 0-9, _, - и его длина должна быть <= 121.

Также должна быть реализована команда `include_dir путь_до_директории`. Данная команда **сразу** читает все файлы конфигурации
 в заданной директории в лексикографическом порядке. Если текущий файл есть указанной директории, то он должен быть проигнорирован.

> [!NOTE]
>  Если в файле конфигурации или последовательности открытых при помощи include_dir файлов конфигурации несколько раз встретилось
>  присваивание к одному ключу, то используется последнее присвоенное значение.
