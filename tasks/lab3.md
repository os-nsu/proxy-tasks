...
> [!TIP]
> Под корректной обработкой сигналов подразумевается, в том числе, что функция-обработчик будет
>signal-safety (см. man 7 signal-safety). Если код обработчика получается большим и в нём приходится использовать
>не signal-safety функции, то вы, например, можете устанавливать флаг прихода сигнала в обработчике,
>после чего в основном цикле мастер-процесса производить необходимые вам действия.
